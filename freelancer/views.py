...
def freelancer_view(request):
    ...
    # Calculate total revenue from individual projects
    individual_revenue = payment_installments.aggregate(total=Sum('amount'))['total'] or 0
    ...
    # Calculate total revenue from team projects
    team_revenue = salary_payments.aggregate(total=Sum('amount_paid'))['total'] or 0
    
    # Calculate total combined revenue
    total_revenue = individual_revenue + team_revenue
    
    # Calculate total contracted projects (projects with contracts)
    contracted_projects = FreelanceContract.objects.filter(
        Q(freelancer=logged_user) |  # Individual projects
        Q(project__team_id__in=TeamMember.objects.filter(user=logged_user).values('team_id'))  # Team projects
    ).count()
    
    # Calculate number of unique clients worked with
    unique_clients = Project.objects.filter(
        Q(freelancer=logged_user) | 
        Q(team_id__in=TeamMember.objects.filter(user=logged_user).values('team_id'))
    ...
    for installment in payment_installments:
        ...
        if isinstance(month_date, date) and not isinstance(month_date, dt):
...
def single_project_view(request, pid):
    ...
    if profile1.permission:
        ...
        # Get user's subscription (active or free)
        ...
        if not subscription:
            # Create a free tier subscription if user doesn't have any
            subscription = UserSubscription.objects.create(
                user=request.user,
                start_date=timezone.now(),
                end_date=timezone.now() + timezone.timedelta(days=30),
                is_active=True,
                payment_status='Pending'  # Add this for free tier
            )

        # Check if this is a paid subscription
        is_paid_subscription = subscription.payment_status == 'Completed' and subscription.plan is not None

        if not is_paid_subscription:
            # Count proposals submitted during free subscription period
            subscription_proposals = Proposal.objects.filter(
                freelancer=request.user,
                date_issued__gte=subscription.start_date.date(),
                date_issued__lte=subscription.end_date.date(),
                status='Accepted'
            ).count()
            
            if subscription_proposals >= 3:
                subscription_context = {
                    'can_submit_proposal': False,
                    'message': f'You have reached the limit of 3 accepted proposals for your free period. Free tier limit resets on {subscription.end_date.strftime("%B %d, %Y")}. Please subscribe for more proposals.',
                    'redirect_url': reverse('freelancer:plans')
                }
            else:
                proposals_remaining = 3 - subscription_proposals
                days_remaining = (subscription.end_date.date() - timezone.now().date()).days
                subscription_context['message'] = (
                    f'You can submit {proposals_remaining} more proposals in your free period. '
                    f'Free tier resets in {days_remaining} days.'
                )
        else:
            ...
            if timezone.now() > subscription.end_date:
                subscription_context = {
                    'can_submit_proposal': False,
                    'message': f'Your subscription expired on {subscription.end_date.strftime("%Y-%m-%d")}. Please renew to continue.',
                    'redirect_url': reverse('freelancer:plans')
                }
            else:
                # Count only proposals submitted during current subscription period
                subscription_proposals = Proposal.objects.filter(
                    freelancer=request.user,
                    date_issued__gte=subscription.start_date.date(),
                    date_issued__lte=subscription.end_date.date(),
                ...
                if subscription_proposals >= subscription.plan.max_proposals_limit:
                    subscription_context = {
                        'can_submit_proposal': False,
                        'message': f'You have reached your {subscription.plan.name} plan\'s limit of {subscription.plan.max_proposals_limit} accepted proposals for this subscription period.',
                        'redirect_url': reverse('freelancer:plans')
                    }
                else:
                    proposals_remaining = subscription.plan.max_proposals_limit - subscription_proposals
                    days_remaining = (subscription.end_date.date() - timezone.now().date()).days
                    subscription_context['message'] = (
                        f'You can submit {proposals_remaining} more proposals with your current plan. '
                        f'Your subscription is valid for {days_remaining} more days.'
                    )
