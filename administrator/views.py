...
def admin_view(request):
    ...
    # Other counts
    total_complaint = Complaint.objects.all().count()
    ...
    # Events and Portfolio Templates
    total_events = EventAndQuiz.objects.count()
    total_portfolios = Template.objects.count()

    # Monthly user data
    monthly_data = CustomUser.objects.annotate(
        month=TruncMonth('joined')
    ...
    for month in monthly_data:
        ...
        client_counts.append(
            CustomUser.objects.filter(
                role='client', 
                joined__month=month['month'].month, 
                joined__year=month['month'].year
            ).count()
        )
    ...
    # Average rating
    average_rating = SiteReview.objects.aggregate(
        Avg('rating')
    )['rating__avg'] or 0
    ...
    context = {
        ...
        'monthly_ratings': json.dumps(monthly_ratings_data),
...
def get_plan_subscribers(request, plan_id):
    """Get all subscribers for a specific plan"""
    if not request.user.is_staff:
        return JsonResponse({'error': 'Unauthorized'}, status=403)
        
    plan = get_object_or_404(SubscriptionPlan, id=plan_id)
    subscriptions = UserSubscription.objects.filter(plan=plan).select_related('user')
    ...
    for subscription in subscriptions:
        ...
        subscribers.append({
            ...
            'email': user.email,
            'profile_image': register.profile_picture.url if register.profile_picture else None,
            'user_type': user.role.capitalize(),
...
def get_subscriber_details(request, subscriber_id):
    ...
    subscriber_data = {
        ...
        'profile_image': register.profile_picture.url if register.profile_picture else None,
        'user_type': user.role.capitalize(),
        'joined_date': user.joined.strftime('%Y-%m-%d'),
    ...
    return JsonResponse(subscriber_data)

def cancel_subscription(request, subscriber_id):
